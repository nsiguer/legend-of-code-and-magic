...
func (p *Player) PickPolicy(cards []*Card) int {
	...
	ref_type := map[int]int {
		CARD_TYPE_CREATURE: 19,
		CARD_TYPE_ITEM_BLUE: 3,
		CARD_TYPE_ITEM_GREEN: 3,
		CARD_TYPE_ITEM_RED: 4,
	}
...
func SampleGame(path string, id int) (*Player) {
	p1 := NewPlayer(1, STARTING_LIFE, STARTING_MANA, STARTING_RUNES)
	p2 := NewPlayer(0, STARTING_LIFE, STARTING_MANA, STARTING_RUNES)
...
func main() {
	for i := 0 ; i < 10000 ; i++ {
		...
		w := SampleGame(path, i)
		if w != nil {
			new_path = fmt.Sprintf("./games/games-%d-%d", w.Id, t)
		} else {
			new_path = fmt.Sprintf("./games/games-2-%d", t)
		}
